...
public class NationalSearchControllerTest extends WithApplication {
    ...
    public void validUserAndTimeTokenReturns200Response() throws UnsupportedEncodingException {
        ...
        val encryptedUser = URLEncoder.encode(Encryption.encrypt("roger.bobby", secretKey).orElse(""), "UTF-8");
        val encryptedTime = URLEncoder.encode(Encryption.encrypt(String.valueOf(Instant.now().toEpochMilli()), secretKey).orElse(""), "UTF-8");
    ...
    public void validUserAndOldTimeTokenReturns401Response() throws UnsupportedEncodingException {
        ...
        val encryptedTime = URLEncoder.encode(Encryption.encrypt(overAnHourAgo, secretKey).orElse(""), "UTF-8");
    ...
    private Http.RequestBuilder buildIndexPageRequest(int minutesDrift) throws UnsupportedEncodingException {
        ...
        val encryptedUser = URLEncoder.encode(Encryption.encrypt("roger.bobby", secretKey).orElse(""), "UTF-8");
        val encryptedTime = URLEncoder.encode(Encryption.encrypt(String.valueOf(Instant.now().toEpochMilli() + (1000 * 60 * minutesDrift)), secretKey).orElse(""), "UTF-8");
