...
public class NationalSearchController_MaintenanceMode_Test extends WithApplication {
    ...
    private Http.RequestBuilder buildIndexPageRequest() throws UnsupportedEncodingException {
        val encryptedUser = URLEncoder.encode(Encryption.encrypt("roger.bobby", "ThisIsASecretKey").orElse(""), "UTF-8");
        val encryptedTime = URLEncoder.encode(Encryption.encrypt(String.valueOf(Instant.now().toEpochMilli()), "ThisIsASecretKey").orElse(""), "UTF-8");
